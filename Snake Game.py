...
golden_apple_pos = [0, 0]
golden_apple_spawn = False
golden_apple_chance = 5  # Chance of golden apple appearing (in percentage)
...
while True:
    ...
    # Teleporting snake when it goes out of bounds
    if snake_pos[0] < 0:
    ...
    if snake_pos[0] == food_pos[0] and snake_pos[1] == food_pos[1]:
        ...
        # Randomly spawn a golden apple
        if not golden_apple_spawn and random.randint(1, 100) <= golden_apple_chance:
            golden_apple_pos = [random.randrange(1, (frame_size_x // 10)) * 10, random.randrange(1, (frame_size_y // 10)) * 10]
            golden_apple_spawn = True
    else:
        snake_body.pop()

    # Check if the snake has eaten the golden apple
